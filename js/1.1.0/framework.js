...
class HTTPRequest {
    ...
    Get(method){
        ...
        httpRequest.open(method, this.uri);
        httpRequest.send();
        httpRequest.onreadystatechange = ()=>{ this.OnStateChange(httpRequest) };
    }

    OnStateChange(httpRequest){
        if (httpRequest.readyState === XMLHttpRequest.DONE) {
            let stat = httpRequest.status;

            if (stat >= 200 && stat < 300) {
                this.callback(httpRequest.responseText);
            } else if(stat >= 400 && stat < 500){
                this.OnClientError(stat);
            } else if(stat >= 500){
                this.OnServerError(stat);
            }
        }
    }
}
